...
- **P3 · `content-types`-Modul** (umbenannt von `post-type-registry` — Naming-Kollision mit Core-`PostTypeRegistry`, BRIEF § 3.1) (BRIEF-pflichtig, NEU aus E7) ✓ **BRIEF v1.0 FREIGEGEBEN** (2026-06-14, `f746973`): CPT-/Taxonomie-Registrierung ins Plugin (args-treu 1:1) + generischer DB-Store + Importer + Orphan-Detektor. **Default-OFF**, harte Invariante „leerer Store = nichts registrieren". ACF-CPT-Definitionen später deaktivieren (Deployment). Voraussetzung P0. **Code-Phase next** (Reihenfolge BRIEF § 13).
...
**Stand: 2026-07-08 (Live-First-Pivot). Vier Feature-Module + `acf-cleanup` live; NEU: Modul `category-pages` (P8/P9-Umbau) begonnen — Kategorie-Seiten-Engine (Slice 1) live + von Jonas abgenommen. NÄCHSTER: `category-pages` Step 4 „Was koche ich heute" (Filter + REST/Nonce), dann Tag-Gruppen/Static-Page/Settings, dann kadence-child-Gerüst (BRIEF § 8).**

### Session 2026-07-08 (Forts.) — `category-pages`: Theme→Plugin-Umbau begonnen (Kategorie-Seiten-Engine Slice 1 live)
- **BRIEF v1.0 freigegeben** (`modules/category-pages/BRIEF.md`): ein-Durchlauf-Umbau Alt-Theme `alkipedia 3.0` → dünnes `kadence-child` + Modul `category-pages` (Logik). Entscheidungen (Jonas): **OE-1** Konfig-Ansatz A (Meta-Box je Page, opt-in Toggle + geplante zentrale Admin-Liste; KEIN CPT wegen Permalink-Fragilität), **OE-2** Rezeptkategorie Seite-1-Legacy-Verhalten, **OE-3** Favoriten-Archiv exakte Kadence-Loop-Optik (server-gerendert, noch offen).
- **Basis: zwei Deep-Read-Specs** des Alt-Themes (`scratchpad/spec-category-engine.md` + `spec-presentation.md`) — Master-Zuordnung „übernehmen/aus-Plugin/weg" in BRIEF § 3.
- **Kategorie-Seiten-Engine (Slice 1) gebaut + live abgenommen:** `Query/Query_Builder` (pure Multi-Taxonomie-AND/OR, Legacy-`$tag_id`-Bug gefixt) · `Query/Term_Resolver` (liest per-Taxonomie-Felder `df_catpage_terms_{tax}`, dann Legacy `rezept_tag`) · `Support/Taxonomies` (kuratierbare Taxonomien = get_object_taxonomies der supported types) · `Provisioning/Fields` (statische Konfig-Felder + je Taxonomie ein ACF-Multiselect, init prio 20) · `Frontend/Category_Page` (`[df_category_page]` + **Auto-Render** geflaggter Seiten via the_content; Seite-1-Vorschau/Folgeseiten-Grid; Kadence-Loop-Karten; Pagination) · `Frontend/Title` (Custom-H1 via the_title + SEO via rank_math/frontend/title) · `Hooks/Rewrite` (enge per-Seite `/page/N/`-Rewrites + deferred Flush) · `Hooks/Layout` (Sidebar ab Seite 2 aus = Kadence-`_kad_post_layout`→`normal`).
- **Felder auf `page` (opt-in via `df_catpage_enabled`):** `df_catpage_title` (H1/SEO), `df_catpage_related_heading` (H2 Seite 1), `df_catpage_per_page_first`/`df_catpage_per_page`, + dynamisch `df_catpage_terms_{taxonomy}`.
- **Debug-Lehren (Jonas):** (1) mein Deploy-Befehl hatte `acf-cleanup` aus der Modul-Liste gekippt → Admin-Seite „verschwand" (nur deaktiviert, wieder reingenommen). (2) „testbar" zu früh gerufen: Term-Auswahl-UI fehlte, `low-carb`-Fallback maskierte die Leere → Fallback raus, echte per-Taxonomie-UI nachgezogen. (3) CPTs waren NIE das Problem (supported types = post, cocktails, trinkspiele, bar-equipment). (4) Sidebar-Off zuerst `fullwidth` (edge-to-edge, „zerschossen") → korrekt `normal`.
- **Commits:** `08d69f5` (BRIEF v1.0 + Skelett/Query_Builder) · `0ce103d` (Term_Resolver + Shortcode + Rewrite) · `1d4767f` (Term-UI + Auto-Render + Fallback raus) · `712371b` (Titel/H2/Pagination/Sidebar) · `2a96d1e` (Sidebar `normal`).
- **Offen im Modul (BRIEF § 8):** Step 4 „Was koche ich heute" (Filter + REST-Nonce), Step 5 Tag-Gruppen (`tag_group` Term-Meta + Admin-Spalte) + Static-Page-Intro, Step 6 pre_get_posts-Toggles + Settings-Tab + schema-engine `publishingPrinciples`, Step 7 kadence-child-Gerüst, Step 8 Voll-Smoke. Zentrale Admin-Liste (OE-1) + Favoriten-Kadence-Optik (OE-3) noch zu bauen.

### Session 2026-07-08 — Live-First-Pivot: Feature-Module live, ACF aufgeräumt, `acf-cleanup` gebaut
- **PIVOT (verbindlich):** Das Plugin **besitzt die CPTs NICHT** — die bleiben site-seitig in ACF. Das Plugin (a) trägt seine Logik **post-type-agnostisch** (ADR-4, per-CPT aktivierbar) und (b) **legt die nötigen ACF-Felder automatisch an** (der Nutzer erstellt nie manuell ACF-Felder). Der Feldsatz wird aus den Feature-/Legacy-Plugins abgeleitet, NICHT aus dem chaotischen Ist-Zustand gespiegelt. → **`content-types` PARKED** (gebaut/getestet/deployt, aber deaktiviert; nicht gelöscht).
- **Neuer Core-Baustein `src/Support/Fields/Field_Provisioner.php`** (~385 Z., Commit `062668d`): wiederverwendbarer Provisioner. Ein Feature-Modul ruft `new Field_Provisioner($fields, $group)` → registriert Meta (`register_post_meta`/`register_term_meta`/`register_meta('user')`, `show_in_rest`, Sanitize) am `init` + optional ACF-Group am `acf/init` (Key-Reuse-Override). `meta-registry` bleibt einfachandersessen-zentriert und ist auf alkipedia **AUS**.
- **Vier Feature-Module live auf Staging (alle verifiziert):**
  - `language-selector` — link_de/link_en (Post+Term) + `<head>`-hreflang + `[df_language_switcher]`.
  - `favorites` — localStorage-Merkliste + globaler Like-Zähler (`_my_favorite_post_likes`, exakt der Legacy-Key → Daten kamen ohne Migration mit: 23 Posts/28 Likes/Max 5), REST-Toggle mit **Nonce** (schließt die Legacy-Lücke), Legacy-Shortcode-Aliase (`thumbnail_favorite_button` etc.) für Theme-Templates → Herz überm Bild + „Ähnliche Beiträge" zurück.
  - `schema-engine` — Autor-Anreicherung (jobTitle=Barkeeper, alumniOf=European Bartender School/barschool.net, knowsAbout, worksFor) im Rank-Math-Graph **und** WPRM-Recipe; kein Doppel-Schema mehr.
  - `newsletter` — Flodesk-Inserter (the_content) + App-Promo; Spotlight-Grau-Overlay repariert (Wrapper-`z-index:9999` statt 1, damit die Sidebar-App-Box mit-abgedunkelt wird; Commit `3d27bc9`).
- **3 Legacy-Plugins deaktiviert:** category-schema, my-favorite-posts-plugin, spotlight-subscribe. **rest-api-wprm bleibt** (App-Abhängigkeit → P10 `rest-legacy`).
- **ACF aufgeräumt (Staging):** 4 überschattete DB-Duplikate gelöscht (translation/Author/Kategorie/Review), Backup vorher; verifiziert über `acf_get_field_groups()` = die Keys weiterhin `local:yes` (Plugin liefert sie per PHP). Behalten: Rezeptkategorie + Tag Addons (nicht abgedeckt). `group_tag_settings`-„Leak" aufgeklärt = altes Theme `alkipedia 3.0/functions.php`, kein Plugin-Problem.
- **NEU gebaut: Modul `acf-cleanup` (Migrations-Aufräumer)** — BRIEF v1.0 + Code (`manifest`/`module`/`Support/Scanner`/`Support/Backup`/`Admin/Cleanup_Page`). Geführte Admin-Seite: Dry-Run-Vorschau (covered=Duplikat/keep) → Auto-Backup (JSON nach uploads) → Löschen nur der server-seitig neu berechneten `covered`-Menge (Gate: `manage_options`→Nonce→Re-Scan). Fasst CPT/Tax-Defs + nicht-abgedeckte Gruppen NIE an. Default-OFF. php -l clean, Autoloader-Smoke grün. **Noch nicht live getestet.**
- **Arbeitsmodus:** Operator-in-the-loop (SSH aus dem Container unmöglich → Jonas führt wp-cli aus, Claude verifiziert per HTTPS/Playwright, Cache-Buster `?cb=` Pflicht wegen RunCache+Cloudflare).

#### Neue Feature-Wünsche / Backlog (2026-07-08)
- **Likes-Dashboard (Jonas-Wunsch):** Admin-Seite im Plugin, die die Like-Zähler (`_my_favorite_post_likes`) als Ranking/Liste zeigt. Der Zähler ist bewusst nur intern; das Dashboard macht ihn sichtbar. → eigenes kleines Admin-Modul, wenn die Live-Basis steht.
- **Live-Cutover alkipedia.com:** Reihenfolge = (0) Backup → (1) neues Plugin aktiv + Module setzen (ACF/CPTs bleiben) → (2) Alt-Plugins deaktivieren → (3) verifizieren (Cocktails kein 404!) → (4) optional `acf-cleanup` (kosmetisch). ACF-Plugin bleibt DAUERHAFT aktiv.
- **Rest offen (Sprint):** P8 Theme kadence-child (Alt-Theme `alkipedia 3.0` läuft noch), P9 Kategorie-Automatisierung, P10 rest-legacy, P11 cache-bridge/Speed (`depeur-bloat`, eigenes Repo). Modul-Toggle-UI (Task 4b) für UI-Aktivierung statt wp-cli.

### Session 2026-06-14 (h) — P3 `content-types` BRIEF v1.0 FREIGEGEBEN (KEIN Code)
- **Ablauf:** Reorientierung (Files + empirische CPT-/Taxonomie-Enumeration via wp-cli) → 4 Approval-Blöcke (§1–3 · §4–6 · §7–9 · §10–13, je schnelle Freigabe) → konsolidierter Write → Commit `f746973` (`modules/content-types/BRIEF.md`, 414 Z., Schema 1.1). § 12.3-konform: Code erst nach Freigabe.
- **Namens-Entscheidung (Weggabelung B, Jonas-getrieben):** Modul **`content-types`** statt `post-type-registry`. Grund: lexische Kollision mit Core-`PostTypeRegistry` ist Wartbarkeits-Gift — solange der Name noch Doku ist (BRIEF-Phase), an der Wurzel beseitigt, nicht „dokumentiert weg". **Semantischer Split (BRIEF § 3.1):** Core-`PostTypeRegistry` (ADR-4) = *welche registrierten Typen kriegen Features* (Selektor); Modul `content-types` = *welche Typen existieren* (Definer/Registrar). Klassennamen-Prinzip: KEIN `PostType…`/`…Registry`-Token im Modul (`Type_Provider`/`Taxonomy_Provider`/`Store`/`Importer`/`Orphan_Detector`).
- **Empirie-Befund (live, localhost:8889 — Gold, in BRIEF § 2):** 3 ACF-CPTs `cocktails`/`bar-equipment`/`trinkspiele` (alle `hierarchical=true`, unterschiedliche supports/menu_position/has_archive-Form), 3 Taxonomien `anlass`/`art`/`herkunft` (alle an `cocktails`, **`herkunft` public=false**). `blog`/`tests` sind KEINE CPTs (nur Field-Locations). Quelle auslesbar via `acf_get_acf_post_types()`/`get_post_type_object()`. → **args-Treue 1:1 Pflicht** (keine Uniform-Defaults).
- **Eingefrorene Architektur (BRIEF § 3 — die Anker für die Code-Phase):**
  - **A (Persistenz):** DB-Option `depeur_food_content-types` (autoload=yes) + Code-**Seed-Pack** (`config/seed.php`, nicht auto-registriert) als Recovery-/Bootstrap-Quelle.
  - **Harte Invariante (§ 3.3):** leerer/fehlender Store ⇒ **nichts registrieren** (kein Default-Seed; frische Installation legt keine fremden CPTs an). Die zentrale „trau-mich-nicht"-Schutzlinie.
  - **Skip-Guard-Koexistenz (§ 4.5):** `post_type_exists()`/`taxonomy_exists()`-Check vor Registrierung → ACF gewinnt solange aktiv, Modul ruht; nach ACF-Abschaltung übernimmt Modul. Trägt die Deployment-Order.
  - **Orphan-Detektor (§ 3.4):** `admin_notices`-Scan erkennt publizierte Posts nicht-registrierter Typen + Re-Import-Link (gegen das 217-cocktails-404-Albtraum-Szenario). Transient-gecached.
  - **C (ACF-Entkopplung):** ACF nur einmalige Import-Quelle (kein Runtime-Coupling). Zwei gegensätzliche ACF-Beziehungen: meta-registry **behält** ACF (Editor-UI), content-types **löst** ACF ab (Struktur). „ACF abschalten" = ACF-CPT-**Definitionen** deaktivieren, NICHT das ACF-Plugin (bleibt aktiv).
  - **Entkopplung Import↔Features (§ 3.6):** Registrierung ≠ Feature-Aktivierung; neuer CPT erscheint als Kandidat in Core-`get_available()`, aktiviert wird er manuell im Core-Tab (nur Wegweiser, kein Auto-Coupling).
  - **Deployment-Checkliste (§ 8):** Aktivieren → Import → Verifikation → **Re-Import (Drift-Sync) vor ACF-Abschaltung** → ACF-Defs aus → Modul übernimmt → Re-Verifikation.
- **Datei-Liste/Code-Reihenfolge (BRIEF § 13):** `config/seed.php` → `Definitions/Store.php` → `Registration/Type_Provider.php` + `Taxonomy_Provider.php` → `Definitions/Importer.php` + `Registration/Orphan_Detector.php` → `Admin/Settings.php` (Diagnose + Importer-Sektion, Sicherheits-Pfad: Cap→Nonce→Sanitize) → Smoke. **~870–1.000 LOC · 2–3 Sessions · KEINE Core-Datei-Änderungen** (reines Modul; `html`-Feldtyp aus `3b83b4d` reicht).
- **Commit (Session h):** `f746973` (BRIEF v1.0) · `<dieser>` (Handoff).
- **Nächster Schritt:** P3 Code-Phase. **Erster Code-Schritt glasklar:** `config/seed.php` aus dem empirischen § 2-Live-Dump authoren (3 CPTs + 3 Taxonomien, args-treu). Davor Smoke-Step-0-Analog: Autoloader-Auflösung der `ContentTypes`-Klassen verifizieren.
...
- **E7: CPT-Registrierung wandert ins Plugin** (`content-types`-Modul, P3 — umbenannt von `post-type-registry`, BRIEF § 3.1) — „Plugins so weit möglich reduzieren". CPT UI später deaktivieren. ACF-Field-Groups bleiben separat in ACF-UI (NICHT E7-Scope).
