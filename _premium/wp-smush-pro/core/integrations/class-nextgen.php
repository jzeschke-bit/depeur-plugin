...
class NextGen extends Abstract_Integration {
	...
	public function setting_status() {
		return ! Membership::get_instance()->is_pro() ? true : ! $this->enabled;
	}
	...
	private function resize_image( $attachment_id, $image, $meta, $storage ) {
		...
		$is_resized     = $resizer->resize();
		$resized_width  = $resizer->get_new_width();
		$resized_height = $resizer->get_new_height();
		...
		if ( $is_resized ) {
			...
			$meta['width']         = ! empty( $resized_width ) ? $resized_width : $meta['width'];
			$meta['full']['width'] = ! empty( $resized_width ) ? $resized_width : $meta['width'];

			$meta['height']         = ! empty( $resized_height ) ? $resized_height : $meta['height'];
			$meta['full']['height'] = ! empty( $resized_height ) ? $resized_height : $meta['height'];
