<?php
/**
 * Handle the recipe Tumblr share shortcode.
 *
 ...
 */
class WPRM_SC_Tumblr_Share extends WPRM_Template_Shortcode {
	public static $shortcode = 'wprm-recipe-tumblr-share';
	...
	public static function init() {
		self::$attributes = array(
			...
			'text' => array(
				'default' => __( 'Share on Tumblr', 'wp-recipe-maker' ),
				'type' => 'text',
			),
			'text_style' => array(
				'default' => 'normal',
	...
	public static function shortcode( $atts ) {
        ...
        $url = 'https://demo.wprecipemaker.com/amazing-vegetable-pizza/';

		$share_url = 'https://www.tumblr.com/widgets/share/tool?posttype=link';
		...
		if ( $atts['icon'] ) {
			...
			if ( $icon ) {
				$icon = '<span class="wprm-recipe-icon wprm-recipe-tumblr-share-icon">' . $icon . '</span> ';
			}
		}
		...
		if ( 'text' !== $atts['style'] ) {
			$classes[] = 'wprm-recipe-tumblr-share-' . $atts['style'];
		...
		$aria_label = '';
		if ( ! $text ) {
			$aria_label = ' aria-label="' . __( 'Share on Tumblr', 'wp-recipe-maker' ) . '"';
		}

		// Return for use in popup shortcode.
		if ( (bool) $atts['use_in_popup'] ) {
			return '<a href="' . esc_attr( $share_url ) . '" data-recipe="' . esc_attr( $recipe->id() ) . '" class="wprm-recipe-tumblr-share" target="_blank" rel="nofollow noopener"' . $aria_label . '>%wprm_share_placeholder%</a>';
		}
