...
class WPRMPRS_Api_Manage {
	...
	public static function api_register_data() {
		if ( function_exists( 'register_rest_field' ) ) {
			register_rest_route( 'wp-recipe-maker/v1', '/manage/recipe-submission', array(
				'callback' => array( __CLASS__, 'api_manage_recipe_submissions' ),
				'methods' => 'POST',
				'permission_callback' => array( __CLASS__, 'api_required_permissions' ),
			) );
			register_rest_route( 'wp-recipe-maker/v1', '/manage/recipe-submission/bulk', array(
				'callback' => array( __CLASS__, 'api_manage_recipe_submissions_bulk_edit' ),
				'methods' => 'POST',
				'permission_callback' => array( __CLASS__, 'api_required_permissions' ),
			) );
		}
	...
	/**
	 * Handle manage recipe submissions call to the REST API.
	 *
	 * @since    5.0.0
	 * @param    WP_REST_Request $request Current request.
	 */
	public static function api_manage_recipe_submissions( $request ) {
		// Parameters.
		...
		add_filter( 'posts_where', array( __CLASS__, 'api_manage_recipe_submissions_query_where' ), 10, 2 );
		$query = new WP_Query( $args );
		remove_filter( 'posts_where', array( __CLASS__, 'api_manage_recipe_submissions_query_where' ), 10, 2 );
	 ...
	 * @since    5.0.0
	 */
	public static function api_manage_recipe_submissions_query_where( $where, $wp_query ) {
		global $wpdb;
	...
	/**
	 * Handle recipe submissions bulk edit call to the REST API.
	 *
	 * @since    5.5.0
	 * @param    WP_REST_Request $request Current request.
	 */
	public static function api_manage_recipe_submissions_bulk_edit( $request ) {
		// Parameters.
		$params = $request->get_params();
		...
		if ( $ids && $action && $action['type'] ) {
			foreach ( $ids as $id ) {
				switch ( $action['type'] ) {
					case 'approve':
						WPRMPRS_Saver::approve_recipe( $id, false, false );
						break;
					case 'approve-add-post':
						WPRMPRS_Saver::approve_recipe( $id, true, false );
						break;
					case 'delete':
						$post = get_post( $id );

						if ( WPRM_POST_TYPE === $post->post_type && 'pending' === $post->post_status && current_user_can( 'delete_post', $id ) ) {
