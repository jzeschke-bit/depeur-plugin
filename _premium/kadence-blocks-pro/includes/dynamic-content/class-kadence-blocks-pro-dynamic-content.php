...
class Kadence_Blocks_Pro_Dynamic_Content {
			...
			if ( self::POST_GROUP === $group ) {
				...
				if ( $post && is_object( $post ) && ( ( function_exists( 'is_post_publicly_viewable' ) ? is_post_publicly_viewable( $post ) : 'inherit' === $post->post_status || 'publish' === $post->post_status ) || current_user_can( 'read_post', $post->ID ) ) && apply_filters( 'kadence_dynamic_enable_password_content', ! post_password_required( $post ) ) ) {
					switch ( $field ) {
						...
						case 'post_content':
							...
							// Set global $post to the source post so blocks inside the content (e.g. dynamic gallery)
							// resolve "current" to the correct post when rendered via the_content.
							global $wp_query;
							$prev_post       = $GLOBALS['post'];
							$GLOBALS['post'] = $post;
							$prev_query_post = null;
							if ( isset( $wp_query->post ) ) {
								$prev_query_post   = $wp_query->post;
								$wp_query->post    = $post;
							}
							$output = apply_filters( 'the_content', get_the_content( '', false, $post ) );
							$GLOBALS['post'] = $prev_post;
							if ( null !== $prev_query_post ) {
								$wp_query->post = $prev_query_post;
							}
							break;
						case 'has_taxonomy':
							$output = false;
							...
							if ( ! empty( $para ) ) {
								if ( 'kb_custom_input' === $para ) {
									if ( ! empty( $custom ) ) {
										...
										// Check if this is an ACF Google Map field and if so, add logic to return the address string instead of the array of data.
										// This help to ensure consitent map rendering in block on frontend.
										// Google Map acf field schema is defined in acf_field_google_map::get_rest_schema()
										if( $this->is_acf_google_map_field_in_post_meta( $custom, $post->ID ) ) {
				...
				switch ( $field ) {
					...
					case 'archive_description':
						...
						break;
					case 'archive_url':
						$output = get_category_link( $item_id );
						break;
	...
	/**
	 * This function returns boolean if an ACF google map field exists in the post meta for a given meta key and post ID.
	 */
	public static function is_acf_google_map_field_in_post_meta( string $meta_key, $post_id ): bool {
		if( ! function_exists( '\get_field_object' ) || ! $post_id ) {
			return false;
		}
		
		$acf_field = \get_field_object( $meta_key, $post_id );

		$is_valid_field  = is_array( $acf_field ) && ! empty( $acf_field );

		if( ! $is_valid_field ) {
			return false;
		}

		return isset( $acf_field['type'] ) && 'google_map' === $acf_field['type'];
	}
}
